...
class PowerUp:
    ...
    def __init__(self, x: int, y: int, size: int, power_type: PowerUpType):
        ...
        logging.debug(
            f"PowerUp created: type={power_type.name} at ({x}, {y}), size={size}"
        )
    ...
    def _load_sprite(self) -> None:
        ...
        try:
            ...
            sprite_path = os.path.join(
                "assets", "sprites", "power_ups", f"{sprite_name}.png"
            )
            ...
            if os.path.exists(sprite_path):
                ...
                logging.warning(
                    f"Power-up sprite not found at {sprite_path}, using fallback"
                )
                self.sprite = None
...
class SpeedBoost(PowerUp):
    ...
    def apply(self, player) -> None:
        ...
        if not hasattr(player, "step"):
            logging.warning(
                "Player doesn't have 'step' attribute, can't apply speed boost"
            )
            return
...
class RapidFire(PowerUp):
    ...
    def revert(self, player) -> None:
        ...
        if self.original_cooldown is not None and hasattr(player, "missile_cooldown"):
            ...
            logging.info(
                f"Reverted rapid fire, cooldown back to {player.missile_cooldown}"
            )
