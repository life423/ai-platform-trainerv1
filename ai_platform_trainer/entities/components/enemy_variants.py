...
class FastEnemy(EnemyPlay):
    ...
    def __init__(self, screen_width: int, screen_height: int, model=None):
        ...
        logging.info(
            f"Created fast enemy, health={self.health}, speed={self.base_speed}"
        )

    def update_movement(
        self, player_x: float, player_y: float, player_step: float, current_time: int
    ) -> None:
        """
        ...
        if dist > 0:
            ...
            zigzag_factor = math.sin(
                2 * math.pi * self.zigzag_counter / self.zigzag_period
            )
...
class TankEnemy(EnemyPlay):
    ...
    def __init__(self, screen_width: int, screen_height: int, model=None):
        ...
        logging.info(
            f"Created tank enemy, health={self.health}, speed={self.base_speed}"
        )
    ...
    def update_movement(
        ...
        if not self.is_charging:
            ...
            if (
                current_time - self.last_charge_time >= self.charge_cooldown
                and random.random() < 0.02
            ):  # 2% chance per frame when off cooldown
