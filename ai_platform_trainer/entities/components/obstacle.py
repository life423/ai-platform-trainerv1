...
class Obstacle:
    ...
    def __init__(self, x: int, y: int, size: int, destructible: bool = False):
        ...
        logging.debug(
            f"Obstacle created at ({x}, {y}), size={size}, destructible={destructible}"
        )
    ...
    def _load_sprite(self) -> None:
        ...
        try:
            ...
            sprite_path = os.path.join(
                "assets", "sprites", "obstacles", f"{sprite_type}.png"
            )
            if os.path.exists(sprite_path):
                ...
                logging.warning(
                    f"Obstacle sprite not found at {sprite_path}, using fallback"
                )
                self.sprite = None
